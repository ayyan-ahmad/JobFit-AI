...
const CustomMockSimulator = ({ sessionId, questions, onTestFinished }) => {
    ...
    return (
        <div className="max-w-3xl mx-auto my-10 p-8 bg-white/[0.02] rounded-2xl shadow-2xl border border-white/[0.06] backdrop-blur-xl">
            {submitError && (
            ...
            {/* Question Card */}
            <div className="mb-8">
                <h3 className="text-xl font-bold text-gray-100 mb-2">{currentQuestion.question}</h3>
                <p className="text-xs text-gray-500 italic">Intent: {currentQuestion.interviewerIntent}</p>
            </div>
                ...
                {currentQuestion.type === "subjective" && (
                    <div className="space-y-4">
                        <div className="flex items-center space-x-3">
                            <button
                                ...
                                className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium text-sm transition-all duration-150 border ${isRecording
                                        ...
                                        : "bg-white/[0.05] hover:bg-white/[0.1] text-gray-300 border-white/[0.1]"
                                    }`}
                            >
                        ...
                        <textarea
                            ...
                            className="w-full p-4 bg-white/[0.02] border border-white/[0.1] rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all duration-150 resize-none text-gray-200 placeholder-gray-600"
                        />
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {currentQuestion.options.map((option, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <span className={`inline-block mr-3 text-sm ${isSelected ? 'text-indigo-400' : 'text-gray-500'}`}>Option {String.fromCharCode(65 + idx)}:</span>
                                    {option}
                                </button>
                            );
                        })}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        {currentQuestion.options.map((option, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div>
                                        <span className={`inline-block mr-3 text-sm ${isSelected ? 'text-indigo-400' : 'text-gray-500'}`}>Choice {String.fromCharCode(65 + idx)}:</span>
                                        {option}
                                    </div>
            ...
            <div className="flex justify-between items-center border-t border-white/[0.06] pt-6">
                <button
                    ...
                    className={`px-5 py-2.5 font-bold rounded-lg text-sm transition-all duration-150 border ${currentIndex === 0
                            ? "bg-transparent text-gray-600 border-white/[0.02] cursor-not-allowed"
                            : "bg-white/[0.02] hover:bg-white/[0.05] text-gray-300 border-white/[0.06]"
                        }`}
                >
                    ⬅️ Previous
                </button>
                ...
                {currentIndex < questions.length - 1 ? (
                    <button
                        ...
                        className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg text-sm shadow-[0_0_15px_rgba(99,102,241,0.2)] transition-all duration-150 hover:-translate-y-0.5"
                    >
                        Next Question ➡️
                    </button>
                ) : (
                    <button
                        ...
                        disabled={loading}
                        className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg text-sm shadow-[0_0_15px_rgba(16,185,129,0.2)] transition-all duration-150 hover:-translate-y-0.5"
                    >
